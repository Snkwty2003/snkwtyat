...
class AdvancedSearch {
    ...
    init() {
        ...
        if (!this.resultsContainer) {
            console.debug("Search results container not found");
    ...
    handleInput(e) {
        try {
            if (!e || !e.target) {
                console.debug("Invalid event or event target");
                return;
            }
            ...
            console.debug("Error handling search input:", error);
        }
    }
    ...
    async search(term) {
        ...
        try {
            ...
            console.debug("Error in search:", error);
        } finally {
            this.isSearching = false;
        }
    }
    ...
    displayResults(results) {
        try {
            if (!this.resultsContainer) {
                console.debug("Results container not available");
                return;
            }
            ...
            console.debug("Error displaying search results:", error);
        }
    }
    ...
    createResultHTML(result) {
        try {
            if (!result) {
                console.debug("No result provided");
                return "";
            }
            ...
            const category = this.escapeHtml(result.category) || "";
            const id = result.id || "";
            ...
            console.debug("Error creating result HTML:", error);
            return "";
        }
    }
    ...
    showLoading() {
        try {
            if (!this.resultsContainer) {
                console.debug("Results container not available for loading");
                return;
            }
    ...
    showError() {
        try {
            if (!this.resultsContainer) {
                console.debug("Results container not available for error");
                return;
            }
            ...
            console.debug("Error showing error:", error);
        }
    }
    ...
    showNoResults() {
        try {
            if (!this.resultsContainer) {
                console.debug("Results container not available for no results");
                return;
            }
            ...
            console.debug("Error showing no results:", error);
        }
    }
    ...
    handleKeyboard(e) {
        try {
            if (!e || !e.key) {
                console.debug("Invalid keyboard event");
                return;
            }
            ...
            case "ArrowDown":
                e.preventDefault();
                if (!focusedResult && results.length > 0) {
    ...
    selectResult(result) {
        try {
            if (!result || !result.dataset) {
                console.debug("Invalid result provided");
                return;
            }
            ...
            console.debug("Error selecting result:", error);
        }
    }
